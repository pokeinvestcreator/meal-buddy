  ...
  if (stats.total === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-6 text-center">
        ...
        <p className="text-stone-500 text-sm mb-6">Plan some meals to auto-generate your list.</p>
    ...
    <div className="pb-6">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-stone-100 px-4 pt-5 pb-3">
        <div className="flex items-center justify-between mb-2">
          ...
          {stats.checked > 0 && (
            <button onClick={onClearChecked} className="text-xs font-semibold text-stone-500 bg-stone-100 px-3 py-2 rounded-xl">
              Clear ✓
            </button>
          )}
        </div>
          ...
          return (
            <section key={catKey}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-stone-400 ml-auto">{checkedCount}/{items.length}</span>
              </div>
              <div className="space-y-1.5">
                {items.map(item => (
                  <GroceryItem key={item.key} item={item} unitPref={unitPref} onToggle={() => onToggleGrocery(item.key)} />
                ))}
              </div>
            </section>
          )
      ...
      <div className="px-4 mt-6 space-y-2.5">
        ...
        <button
          ...
          className={`w-full py-3.5 rounded-2xl font-semibold text-sm ${confirmClear ? 'bg-red-500 text-white' : 'bg-stone-900 text-white'}`}
        >
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 p-3 rounded-xl border text-left transition-opacity ${
        item.checked ? 'bg-stone-50 border-stone-100 opacity-50' : 'bg-white border-stone-200'
      }`}
    >
